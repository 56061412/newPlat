...
public class SnsTaskAction extends BaseAction {
	...
	public String getTaskInfo(){
		...
		try {
			taskInfo = SnsTaskServiceFactory.snsTaskInfoService.queryTaskInfo(taskGuid);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * 获取任务详细
	 */
	public String listTaskPerson(){
		String taskGuid = getRequest().getParameter("taskGuid");
		List<TaskPerson> list = null;
		try {
			list = SnsTaskServiceFactory.snsTaskPersonService.getList(taskGuid);
			for (TaskPerson taskPerson : list) {
				if(taskPerson.getFankuiTime()!=null){
					taskPerson.setExtra1(DateUtil.toDateString(taskPerson.getFankuiTime(), DateUtil.DEFAULT_DATETIME_PATTERN));
				}else{
					taskPerson.setExtra1("");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
